...
def make_crossbar(weights, input_image, num_rows = 10, num_cols = 784):
    ...
    netlist_file=open(file="./interference_crossbar.net", mode="w")
    ...
    current_excitation_node = 0
    
    #Input current sources
    for i in range(num_rows):
        if i == current_excitation_node:
            netlist_file.write(f"I_IN_BACK_{i} VDD IN_BACK_{i} {current_pulse_string}\n")
        else:
            netlist_file.write(f"I_IN_BACK_{i} VDD IN_BACK_{i} 0\n")
            
    #Input LIF+Tempotron
    netlist_file.write("* Input LIF and Tempotron Nodes\n")
    for i in range(num_rows):
        netlist_file.write(f"XL_LT2_{i} IN_BACK_{i} V_LT2_{i} liftest_mod\n")
        netlist_file.write(f"XT_LT2_LT1_{i} V_LT2_{i} ROW_LT2_{i} ROW_LT2_{i}p tempotroncircuit_new\n")
        
    #Resistor crossbar
    netlist_file.write("* Crossbar Resistors\n")
    offset_res_val = 332 #For spintronics
    for i in range(num_rows):
        for j in range(num_cols):
            synaptic_resistance = 1/((weights[i][j]/900 + 1)/332)
            netlist_file.write(f"R_LT2_LT1_{i}{j} ROW_LT2_{i} COL_LT1_{j} {synaptic_resistance}\n") #CHANGE 332 to a matrix value of your choice
            netlist_file.write(f"R_LT2_LT1_{i}{j}p ROW_LT2_{i}p COL_LT1_{j} {offset_res_val}\n") #Offset resistor
            
    '''
    #Sense Resistors, Behavioral Current Sources and output LIF units at the ends of the column
    netlist_file.write("* Sense Resistors\n")
    for i in range(num_cols):
        netlist_file.write(f"RS{i} COL{i} 0 1p\n")
        netlist_file.write(f"B{i} VDD BXL{i} I = I(RS{i})\n")
        netlist_file.write(f"XLO{i} BXL{i} OUT{i} liftest\n")
    '''
    
    #Current Controlled Current Sources to the next neuron layer at the ends of the column
    netlist_file.write("* CCCS and LT1 LIF\n")
    for i in range(num_cols):
        netlist_file.write(f"XCCCS_LT2_LT1_{i} COL_LT1_{i} BXL_LT1_{i} i_controlled_i_source\n")
        netlist_file.write(f"XL_LT1_{i} BXL_LT1_{i} V_LT1_{i} liftest_mod \n")
        
    #Tempotron, Synaptic Resistors, I2I source, WM LIF neurons
    netlist_file.write("*LT1-WM Tempotron, Synapse, CCCS, WM LIF\n")
    
    for i in range(num_cols):
        netlist_file.write(f"XT_LT1_WM_{i} V_LT1_{i} ROW_LT1_{i} ROW_LT1_{i}p tempotroncircuit_new\n")
        lt1_wm_weight = 600
        lt1_wm_synaptic_resistance = 1/((lt1_wm_weight/900 + 1)/332)
        netlist_file.write(f"R_LT1_WM_{i}{i} ROW_LT1_{i} COL_WM_{i} {lt1_wm_synaptic_resistance}\n") 
        netlist_file.write(f"R_LT1_WM_{i}{i}p ROW_LT1_{i}p COL_WM_{i} {offset_res_val}\n") #Offset resistor
        netlist_file.write(f"XCCCS_LT1_WM_{i} COL_WM_{i} BXL_WM_{i} i_controlled_i_source\n")
...
make_crossbar(weights[:, :, 0:1].reshape((num_rows, -1)), weights[:, :, 1:2].reshape((num_cols, -1)), num_rows = num_rows, num_cols = num_cols)
...
weights_in = np.array([0, 20]).reshape((2, 1))
make_crossbar(weights, weights_in, num_rows = num_rows, num_cols = num_cols)
'''  

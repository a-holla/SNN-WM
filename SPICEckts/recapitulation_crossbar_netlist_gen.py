...
def make_crossbar(weights, num_rows = 10, num_cols = 784):
    
    netlist_file=open(file="./recapitulation_crossbar.net", mode="w")
    ...
    netlist_file.write("* Input Current Sources\n")
    t_exposure = "110u"
    t_observation = "330u"
    t_exposure_int = 110
    ...
    for i in range(num_rows):
        if i == current_excitation_node:
            netlist_file.write(f"I{i} VDD IN{i} {current_pulse_string}\n")
        else:
            netlist_file.write(f"I{i} VDD IN{i} 0\n")
            
    #Input LIF+Tempotron
    netlist_file.write("* Input LIF and Tempotron Nodes\n")
    for i in range(num_rows):
        netlist_file.write(f"XL_LT2_{i} IN{i} V_LT2_{i} liftest_mod\n")
...
make_crossbar(weights[:, :, 0:1].reshape((num_rows, -1)), num_rows = num_rows, num_cols = num_cols)
'''
...
make_crossbar(weights, num_rows = num_rows, num_cols = num_cols)
